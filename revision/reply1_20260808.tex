...
\textbf{\textcolor{ourcol}{Internal note --- delete before submission.}}
Blocks marked \textbf{\textcolor{ourcol}{Our reply (independent draft)}} are
written independently of the draft replies above and are based on our own
re-analysis of the data. Each states the finding, points to the supporting
table or figure, and closes with a one-line note on what it does to the
paper's story. Supporting output is stored under
\texttt{revision/snapshots/}.\\[2pt]
...
\begin{ourreply}
We agree, and this comment changed the inference in the paper more than any
other.

We first did what the referee asks. Estimating crossed random effects
...
We then found that a match random intercept is the right tool for only one of
...
\begin{table}[h]
\centering
\caption{Where a match random effect works and where it does not}
\label{tab:r1c2}
\small
\begin{tabular}{llrl}
\toprule
Equation & Dependent variable & \% between & Inference \\
\midrule
Close-to-end returns (Table~3) & $R_{\text{close-to-end}}$  & 99.7  & crossed RE \\
Relative accuracy (Table~4)    & $|e_0| - |e_T|$            & 82.5  & crossed RE \\
...
Three concrete cases. In the close-to-end return the value is exactly $-1$
whenever the bet loses, whatever the bookmaker or the closing price; 53\% of
matches have exactly zero within-match variance. This is a property of the
return definition, not evidence that bookmakers converge, and we now say so
explicitly. In Equation~3 the dependent variable is the match outcome itself,
which is constant across bookmakers by construction: none of the 20{,}588
matches has more than one value. In the unbiasedness regression a match
effect drives the residual standard deviation down to 0.015 out of 0.459 and
moves $\beta_1$ from 0.988 to 0.466 --- a different estimand, not a more
precise estimate of the same one, and the covariance structure becomes
singular.

We therefore use the referee's own fallback, cluster-robust inference at the
match level, wherever the match effect is not identified. The referee is
right about the direction, and the size of the correction is large: standard
errors rise by a factor of \textbf{3.0} against the model-based ones in the
unbiasedness regression (6.3 against i.i.d.\ standard errors) and by
\textbf{3.0 to 4.1} in Equation~3. We validated the sandwich against a
cluster bootstrap over matches, because our clusters are unbalanced (1 to 24
bookmakers per match, median 7). The two agree at the edges of the betting
window and the bootstrap is 10--22\% wider in the middle, so we report the
...
On the last sentence of the comment, dependence from tournaments or players:
we have not tested it. Clustering at the match level captures the channel the
referee describes, namely a shared terminal outcome. Dependence across
matches of the same player or tournament is a separate channel that our
current inference does not cover, and we will report it.
