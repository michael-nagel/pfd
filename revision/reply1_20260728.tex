...
\ReplyMN{Two of the four features the referee lists are settled elsewhere in this revision: all prices are now margin-adjusted (Comment~1), and the censoring does not reach the learning-rate estimate, whose support points lie in the closing half of the window and never touch the stretch that a truncated series leaves unobserved (Comment~5).

Irregular observation is handled inside the framework. Biais et al.\ (1999, p.~1240, eq.~13) show that the moment condition is invariant to a rescaling of the time axis, and this survives a match-specific clock rate, because the residual already has mean zero row by row and the scale factor therefore cancels before anything is pooled. Empirically the estimate settles once the support points are three grid points apart or more; at tighter spacings the standard error exceeds the estimate itself and a third of the bookmaker estimates turn negative (Table~\ref{tab:r1c6}), which is the referee's concern in reverse and is also the reason Biais et al.\ space their own support points.

The binary terminal value is the assumption we cannot dispose of. In Biais et al.\ the terminal value is latent and the variance of the proxy error enters the moment condition as a second parameter $K$; because the match outcome is observed here, that error is zero by construction, we set $K = 0$ and estimate $\gamma$ alone. We now state this in Section~3.5 as an assumption we make, not as something the transfer delivers.

The transfer was in any case not obvious, and checking it turned up an error of our own. The decay factor was tied to the length of the percentile grid rather than to the positions of the support points actually used,
\begin{align*}
\text{as submitted:} \quad & \mathrm{E}\Big[ (p_{\tau_1} - \omega)^2
- \big( \tfrac{n-1}{n} \big)^{2\gamma} (p_{\tau_2} - \omega)^2 \,\Big|\, I \Big] = 0, \\
\text{corrected:} \quad & \mathrm{E}\Big[ (p_{\tau_1} - \omega)^2
- \big( \tfrac{\tau_2}{\tau_1} \big)^{2\gamma} (p_{\tau_2} - \omega)^2 \,\Big|\, I \Big] = 0,
\qquad \tau_i = n - i \cdot \text{incr} + 1,
\end{align*}
so that at the spacing we use the factor is $42/47 = 0.894$ instead of $50/51 = 0.980$; the two coincide exactly at a spacing of one, which is why this went unnoticed. Correcting it moves the level of the learning rate --- the average over bookmakers falls from 0.032 to 0.005 --- while the ordering across bookmakers (Spearman 0.88), the two extreme bookmakers and the significance pattern (16 of 24) are unchanged, so the cross-sectional statements in the paper stand and only their scale is restated.

\begin{table}[H]
\centering
\caption{The learning rate at different spacings of the support points, estimated per bookmaker and averaged over the 24. Under the submitted decay factor the same series gave 0.0051, 0.0016 and 0.0293 at spacings of one, two and five.}
\label{tab:r1c6}
\small
\setlength{\tabcolsep}{4pt}
\begin{tabular}{lrrrrrrrr}
\toprule
Spacing & 1 & 2 & 3 & 4 & 5 (used) & 6 & 8 & 10 \\
\midrule
$\gamma$              & 0.0057 & 0.0009 & 0.0035 & 0.0037 & 0.0054 & 0.0048 & 0.0040 & 0.0034 \\
Mean standard error   & 0.0095 & 0.0048 & 0.0034 & 0.0027 & 0.0022 & 0.0019 & 0.0015 & 0.0012 \\
Negative estimates    & 7 & 10 & 2 & 3 & 0 & 0 & 1 & 0 \\
\bottomrule
\end{tabular}
\end{table}
}
