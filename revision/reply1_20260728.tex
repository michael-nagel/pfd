...
\ReplyMN{We examined both routes the referee proposes. Crossed random effects with a match intercept turn out not to be usable in these specifications, because the match outcome enters the dependent variable directly: between 82.6\% and 100\% of its variance lies between matches (last column of Table~\ref{tab:r1c2}), so a match intercept absorbs the dependent variable rather than the dependence, and the fitted variance components degenerate --- the core coefficient flips from $+0.028$ to $-0.220$ in Eq.~1 and from $-0.0039$ to $+0.0197$ in Eq.~2. We therefore follow the referee's second route and use cluster-robust inference at the match level throughout, keeping the specification of the submitted version and changing only the covariance matrix,
\begin{align*}
\text{submitted:} \quad & \widehat{\mathrm{Var}}(\hat{\beta})_{\text{model}}, \\
\text{revised:} \quad & \widehat{\mathrm{Var}}(\hat{\beta}) = c \, (X'X)^{-1}
\Big[ \sum\nolimits_{m} X_m' \hat{u}_m \hat{u}_m' X_m \Big] (X'X)^{-1},
\end{align*}
where $m$ indexes matches. The point estimates are untouched in Eq.~1--3; for the unbiasedness regression the sandwich runs on the OLS counterpart, which reproduces the mixed-model coefficient at 42 of the 50 percentiles (median difference 0.007, the deviations confined to the earliest percentiles). For Eq.~2 we additionally cluster on bookmaker, because there the bookmaker heterogeneity is demonstrably real (joint Wald test $\chi^2(46) = 507.7$, $p < 0.0001$), whereas it is not detectable in Eq.~1 ($p = 0.127$) or Eq.~3 ($p = 0.225$); this raises the standard error on the price-change coefficient from 0.00036 to 0.00050 and its $t$-statistic falls from $-11.9$ to $-8.0$. The referee's concern is confirmed quantitatively: the standard errors were too small by a factor of roughly two to four (Table~\ref{tab:r1c2}). The consequence is concrete in Eq.~1, but it falls on the controls rather than on the result: the core coefficient RtrnOpnCls was already insignificant in the submitted version --- that close-to-end returns are not predictable from open-to-close returns is the finding of Table~3, and clustering only reinforces it --- whereas the covariates do lose their significance (Compet\_ITF\_Men from $z = -6.7$ to $t = -1.9$, Compet\_WTA from $-4.9$ to $-1.2$). The central findings of the other equations survive in sign and magnitude.

\begin{table}[H]
\centering
\caption{Model-based versus match-clustered inference. The point estimates are identical in both columns; only the covariance matrix differs.}
\label{tab:r1c2}
\small
...
Eq.~3                 & OpnOdds                  & 1.1248 & 0.0058 & 0.0172 & 3.0 & 100.0\% \\
...
\midrule
Raw scrape                                          & 712{,}151 & --- \\
Final result posted, 2023 onward, consistency checks & 587{,}414 & 17.5\% \\
Margin between 0 and 15\%                            & 585{,}938 & 0.3\% \\
Bookmakers in the lowest size quartile               & 495{,}039 & 15.5\% \\
\textbf{Betting window 12 to 72 hours}               & \textbf{184{,}415} & \textbf{62.8\%} \\
Zero price variation                                 & 184{,}112 & 0.2\% \\
Fewer than 20 price changes                          & 175{,}266 & 4.8\% \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{The coefficient path $\beta_1$ at alternative filter thresholds. Estimated on the continuous specification, which additionally discards updates recorded after kickoff; we describe that specification in more detail in our reply to Comment~7.}
\label{tab:r1c5b}
\small
\setlength{\tabcolsep}{5pt}
\begin{tabular}{lrrrrr}
\toprule
Hours before kickoff & $< 10$ & $< 15$ & $< 18$ & $< 20$ (used) & all series \\
\midrule
24    & 0.995 & 1.134 & 1.206 & 1.241 & 1.251 \\
6     & 0.868 & 0.910 & 0.978 & 1.029 & 0.973 \\
1     & 0.797 & 0.731 & 0.794 & 0.874 & 0.904 \\
0.25  & 0.793 & 0.716 & 0.788 & 0.866 & 0.909 \\
\midrule
Series & 123{,}795 & 157{,}065 & 168{,}489 & 175{,}166 & 184{,}012 \\
\bottomrule
\end{tabular}
\end{table}
}
