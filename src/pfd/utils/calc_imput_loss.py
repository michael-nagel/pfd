...
def calc_imput_loss(
    ...
    mse_med = root_mean_squared_error(labels, imp_med[odds_mvt_cols[:n_mvt]])
    ...
    mse_lin = root_mean_squared_error(labels, imp_lin[odds_mvt_cols[:n_mvt]])
    ...
    mse_mul = root_mean_squared_error(
        labels, df_imp_mul.loc[random_rows.index, odds_mvt_cols[0:n_mvt]]
    )
