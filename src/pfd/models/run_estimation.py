...
def run_estimation(cfg: PFDConfig) -> None:
    ...
    df = pd.read_parquet(f"{cfg.paths.data_proc}shaped_data.parquet")
    ...
    # )
    df["SqErr"] = (df["Match"] - df["OpnOdds"]) ** 2
    rmse = df.groupby("Bookies")["SqErr"].mean().pow(0.5)
    df.drop(columns=["SqErr"], inplace=True)
    ...
    print(f"Cleaned {unreachable_count} objects.")
    ...
    # Initialize pandarallel (progress_bar=True helps track status)
    # nb_workers defaults to all available cores, or set specific count if needed
    pandarallel.initialize(
        progress_bar=False, nb_workers=cfg.sampling.n_cores, verbose=1
    )

    # Efficient parallel resampling
    # parallel_apply automatically handles the splitting and recombination
    df = df.groupby("GroupId").parallel_apply(
        resample,
    ...
    df = df.reset_index(level=1, drop=True).reset_index(drop=True)
    ...
    # Store DataFrame
    df.to_parquet(
        f"{cfg.paths.data_intrm}data_resampled.parquet",
        engine="pyarrow",
        compression="snappy",
    )
    ...
    if cfg.general.save:
        # DataFrames
        df_desc.to_parquet(
            path=f"{cfg.paths.data_proc}data_desc.parquet",
        )
        ...
        metrics.to_parquet(path=f"{cfg.paths.data_proc}metrics.parquet")
